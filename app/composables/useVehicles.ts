...
export function mileageSourceLabel(value: VehicleMileageHistory['source_type']) {
...
export function useVehicles() {
  ...
  const vehiclesState = useAPI<ApiEnvelope<Vehicle[]>>('/vehicles', {
    key: 'vehicles:list',
    immediate: false,
    server: false,
    default: () => [],
  ...
  const filteredVehicles = computed(() => {
    ...
    return list.filter((vehicle) => {
      ...
      return (
        plate.includes(term) ||
        normalizePlate(vehicle.license_plate).toLowerCase().includes(term.replace(/\s+/g, '')) ||
        model.includes(term) ||
  ...
  const updateAndRefreshVehicle = async (id: number, payload: Omit<VehicleFormPayload, 'customer_id'>) => {
...
export function useVehicleActions() {
  ...
  const updateVehicle = async (id: number, payload: Omit<VehicleFormPayload, 'customer_id'>) => {
    const response = await useApiFetch<ApiEnvelope<Vehicle>>(`/vehicles/${id}`, {
      method: 'PUT',
      body: {
        license_plate: normalizePlate(payload.license_plate),
        model: payload.model?.trim() || null,
...
export function useVehicle(vehicleId: Ref<number | null> | ComputedRef<number | null>) {
  const auth = useAuth();

  const vehicle = useAPI<ApiEnvelope<Vehicle>>(
...
export function useMileageHistory(vehicleId: Ref<number | null> | ComputedRef<number | null>) {
  const auth = useAuth();

  const mileageState = useAPI<ApiEnvelope<VehicleMileageHistory[]>>(
  ...
  const updateMileage = async (id: number, payload: MileageFormPayload) => {
    const response = await useApiFetch<ApiEnvelope<VehicleMileageHistory>>(`/vehicle-mileage-history/${id}`, {
      method: 'PUT',
      body: {
        mileage: payload.mileage,
        source_type: 'manual',
