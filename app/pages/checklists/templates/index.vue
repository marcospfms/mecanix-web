...
const isLoading = computed(
  () => status.value === 'pending' && templates.value.length === 0
)
...
const openDetails = async (template: ChecklistTemplate) => {
  await navigateTo({
    name: 'checklists-templates-templateId',
...
const handleSubmit = async () => {
  ...
  try {
    ...
    toast.error({
      ...
      description: getErrorMessage(err, 'Não foi possível salvar o template.')
    })
  } finally {
      ...
      >
        <div class="space-y-2">
          ...
          <p class="max-w-2xl text-sm leading-6 text-toned">
            Estruture os modelos de inspeção usados pela operação e mantenha os
            itens prontos para execução no app.
          </p>
        </div>
    ...
    <AppConfirm
      ...
      :description="
        templatePendingDeletion
          ? `Você está removendo ${templatePendingDeletion.name}. Esta ação não pode ser desfeita.`
