...
const employeeFilterItems = computed<SelectItem[]>(() => {
  ...
  for (const employee of stats.value?.checklists_by_employee ?? []) {
    items.push({
      label: employee.name ?? employee.username ?? `Funcionário #${employee.user_id}`,
...
const selectedEmployeeValue = computed({
  get: () => (selectedEmployeeUserId.value ? String(selectedEmployeeUserId.value) : 'all'),
  set: (value: string) => {
...
const summaryCards = computed(() =>
  [
    stats.value?.total_companies !== undefined
      ? { key: 'companies', label: 'Empresas', value: stats.value.total_companies, icon: 'i-lucide-building-2' }
      : null,
    stats.value?.total_customers !== undefined
      ? { key: 'customers', label: 'Clientes', value: stats.value.total_customers, icon: 'i-lucide-users' }
      : null,
    stats.value?.total_vehicles !== undefined
      ? { key: 'vehicles', label: 'Veículos', value: stats.value.total_vehicles, icon: 'i-lucide-car-front' }
      : null,
    stats.value?.checklists_templates !== undefined
      ? { key: 'templates', label: 'Templates', value: stats.value.checklists_templates, icon: 'i-lucide-layers-3' }
      : null
...
<template>
  <div class="space-y-5 sm:space-y-6">
    ...
    <template v-else-if="stats">
      <UCard class="rounded-[1.75rem] border-default bg-default">
        <div class="space-y-5">
          <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div class="space-y-2">
              <p class="text-sm font-medium text-highlighted">Painel do período</p>
              <p class="text-sm leading-6 text-toned">
                Consolidação do mês com foco em volume, inspeções realizadas e ritmo de execução.
              </p>
            </div>

            <div v-if="employeeStats.length > 0" class="w-full lg:w-72">
              <USelect
          ...
          <div class="grid gap-3 sm:grid-cols-2">
            ...
            >
              <div class="flex items-start justify-between gap-3">
                <div class="space-y-1">
                  <p class="text-sm text-toned">{{ card.label }}</p>
                  <p class="text-2xl font-semibold tracking-tight text-highlighted">{{ card.value }}</p>
                </div>
          ...
          <div class="rounded-2xl border border-default bg-muted/25 p-4">
            <div class="flex items-center justify-between gap-3">
              <div>
                <p class="text-sm font-medium text-highlighted">Concluídos no período</p>
                <p class="text-sm text-toned">
                  {{ stats.checklists_completed }}/{{ stats.checklists_total }} checklists finalizados
                </p>
              </div>

              <p class="text-lg font-semibold text-highlighted">{{ progressValue }}%</p>
            </div>
      ...
      <section class="space-y-3">
        <div class="flex items-center justify-between gap-3">
          <h2 class="text-lg font-semibold text-highlighted">Base operacional</h2>
          <p
      ...
      <section class="space-y-3">
        <h2 class="text-lg font-semibold text-highlighted">Últimas execuções</h2>
        ...
        <div v-else class="grid gap-3">
          ...
          >
            <div class="space-y-4">
              <div class="flex items-start justify-between gap-3">
                <div class="space-y-1">
                  <p class="text-base font-semibold text-highlighted">{{ execution.name }}</p>
                  <p class="text-sm text-toned">
                    {{ execution.vehicle?.customer_name || 'Cliente não identificado' }}
                  </p>
                </div>
                ...
                <span
                  ...
                  "
                >
                  {{ execution.status === 'completed' ? 'Concluído' : 'Rascunho' }}
                </span>
              </div>
              ...
              <div class="grid gap-3 text-sm text-toned sm:grid-cols-3">
                ...
                <div>
                  ...
                  <p>{{ execution.executed_by?.name || 'Sem responsável' }}</p>
                </div>
                <div>
                  <p class="font-medium text-highlighted">Data</p>
                  <p>{{ formatDateTime(execution.executed_at ?? execution.created_at) }}</p>
                </div>
              </div>
            </div>
          </UCard>
      ...
      <section v-if="employeeStats.length > 0" class="space-y-3">
        <h2 class="text-lg font-semibold text-highlighted">Por funcionário</h2>
            ...
            <div class="flex items-center justify-between gap-3">
              <div class="space-y-1">
                <p class="text-base font-semibold text-highlighted">
                  {{ employee.name || employee.username || `Funcionário #${employee.user_id}` }}
                </p>
                <p class="text-sm text-toned">
                  {{ employee.username ? `@${employee.username}` : 'Equipe da operação' }}
                </p>
              </div>

              <div class="rounded-2xl bg-primary/10 px-4 py-2 text-center">
                <p class="text-xs font-medium uppercase tracking-[0.2em] text-primary">Total</p>
                <p class="text-lg font-semibold text-primary">{{ employee.total }}</p>
              </div>
            </div>
          </UCard>
        </div>
