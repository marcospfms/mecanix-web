...
import { formatTaxId, useCustomers } from '../composables/useCustomers';
import { formatLicensePlate, useVehicles } from '../composables/useVehicles';
...
const isLoading = computed(() => status.value === 'pending' && vehicles.value.length === 0);
const isListRefreshing = computed(() => manualRefreshing.value);
const hasVehicles = computed(() => vehicles.value.length > 0);
const displayedVehicles = computed(() => vehicles.value.slice(0, visibleCount.value));
const hasMoreVehicles = computed(() => vehicles.value.length > visibleCount.value);
const formTitle = computed(() => (formMode.value === 'create' ? 'Novo veículo' : 'Editar veículo'));
...
const handleSubmit = async () => {
  ...
  try {
    ...
    toast.error({
      ...
      description: err?.data?.message ?? err?.message ?? 'Não foi possível salvar o veículo.'
    });
  } finally {
      ...
      <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div class="space-y-2">
          ...
          <p class="max-w-2xl text-sm leading-6 text-toned">
            Centralize os veículos cadastrados, acompanhe o vínculo com cada cliente e mantenha a
            base operacional pronta para novas inspeções.
          </p>
        </div>
