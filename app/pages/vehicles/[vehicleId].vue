...
import { formatTaxId } from '../../composables/useCustomers';
...
const mileageFormTitle = computed(() =>
  mileageMode.value === 'create' ? 'Registrar quilometragem' : 'Editar quilometragem'
);
...
const resetVehicleForm = () => {
  licensePlate.value = vehicle.value?.license_plate ? formatLicensePlate(vehicle.value.license_plate) : '';
  model.value = vehicle.value?.model ?? '';
  modelYear.value = vehicle.value?.model_year ? String(vehicle.value.model_year) : '';
...
const handleVehicleSubmit = async () => {
  ...
  try {
    ...
    toast.error({
      ...
      description: err?.data?.message ?? err?.message ?? 'Não foi possível atualizar o veículo.'
    });
  } finally {
...
const handleMileageSubmit = async () => {
  ...
  try {
    ...
    toast.error({
      title: 'Falha ao salvar',
      description: err?.data?.message ?? err?.message ?? 'Não foi possível salvar a quilometragem.'
    });
  } finally {
...
const handleVehicleDelete = async () => {
  ...
  try {
    ...
    toast.error({
      ...
      description: err?.data?.message ?? err?.message ?? 'Não foi possível excluir o veículo.'
    });
  } finally {
...
const handleMileageDelete = async () => {
  ...
  try {
    ...
    toast.error({
      title: 'Falha ao excluir',
      description: err?.data?.message ?? err?.message ?? 'Não foi possível excluir o registro.'
    });
  } finally {
  ...
  <div class="space-y-5 sm:space-y-6">
      ...
      <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div class="space-y-2">
          <h1 class="text-3xl font-semibold tracking-tight text-highlighted">
            {{ vehicle ? formatLicensePlate(vehicle.license_plate) : 'Detalhes do veículo' }}
          </h1>
          <p class="max-w-2xl text-sm leading-6 text-toned">
            Consulte o cadastro do veículo, acompanhe o cliente vinculado e mantenha o histórico
            manual de quilometragem atualizado.
          </p>
        </div>
    ...
    <template v-else-if="vehicle">
        ...
        <div class="space-y-5">
          <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div class="space-y-3">
              <div class="flex flex-wrap items-center gap-2">
                ...
                <span class="rounded-full border border-default px-3 py-1 text-xs font-medium text-toned">
              ...
              <div class="grid gap-4 sm:grid-cols-2">
                <div>
                  <p class="text-xs font-semibold uppercase tracking-[0.24em] text-primary">Placa</p>
                ...
                <div>
                  <p class="text-xs font-semibold uppercase tracking-[0.24em] text-primary">Última quilometragem</p>
                  <p class="mt-1 text-base text-highlighted">
                ...
                <div>
                  <p class="text-xs font-semibold uppercase tracking-[0.24em] text-primary">Ano</p>
                  <p class="mt-1 text-base text-highlighted">{{ vehicle.model_year || '-' }}</p>
                </div>

                <div>
                  <p class="text-xs font-semibold uppercase tracking-[0.24em] text-primary">Cor</p>
                  <p class="mt-1 text-base text-highlighted">{{ vehicle.color || '-' }}</p>
                </div>
              </div>
            </div>

            <div class="flex flex-wrap gap-2 sm:justify-end">
              <UButton color="neutral" variant="soft" icon="i-lucide-pencil" @click="openVehicleEdit">
                Editar
              </UButton>
              <UButton color="primary" icon="i-lucide-gauge" @click="openMileageCreate">
                Registrar km
              </UButton>
              <UButton color="error" variant="soft" icon="i-lucide-trash" @click="askVehicleDelete">
                Excluir
              </UButton>
            </div>
          </div>
          ...
          >
            <div class="flex items-start justify-between gap-3">
              <div>
                <p class="text-xs font-semibold uppercase tracking-[0.24em] text-primary">Cliente</p>
                <p class="mt-1 text-base font-medium text-highlighted">{{ vehicle.customer.name }}</p>
              ...
              <UButton
                ...
                :to="{ name: 'customers-customerId', params: { customerId: String(vehicle.customer.id) } }"
              >
                Ver cliente
              </UButton>
            </div>
      ...
      <section class="space-y-3">
        <div class="flex items-center justify-between gap-3">
          <div>
            <h2 class="text-lg font-semibold text-highlighted">Histórico de quilometragem</h2>
            <p class="text-sm text-toned">
              Registros manuais e leituras geradas por checklist aparecem nesta linha do tempo.
            </p>
          </div>
        </div>
        ...
        >
          <div class="pt-2">
            <UButton color="primary" icon="i-lucide-plus" @click="openMileageCreate">
          ...
          >
            <div class="space-y-4">
              <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                ...
                <div v-if="item.source_type === 'manual'" class="flex gap-2 sm:justify-end">
                  <UButton
                    color="neutral"
                    variant="soft"
    ...
    >
      ...
      <template #footer>
        ...
        <UButton color="primary" :loading="vehicleFormSubmitting" @click="handleVehicleSubmit">
          Salvar alterações
        </UButton>
      </template>
    </USlideover>
    ...
    >
        ...
        <div class="space-y-4">
          <div class="space-y-2">
            ...
            <UInput
              ...
              inputmode="numeric"
            />
            <p v-if="mileageFormErrors.mileage" class="text-sm text-error">
      ...
      <template #footer>
        <UButton color="neutral" variant="ghost" @click="mileageFormOpen = false">
          Cancelar
        </UButton>
        <UButton color="primary" :loading="mileageFormSubmitting" @click="handleMileageSubmit">
          {{ mileageMode === 'create' ? 'Registrar quilometragem' : 'Salvar alterações' }}
        </UButton>
      </template>
    </USlideover>
    ...
    <AppConfirm
      ...
      :description="vehicle ? `Você está removendo ${formatLicensePlate(vehicle.license_plate)}. Esta ação não pode ser desfeita.` : 'Esta ação não pode ser desfeita.'"
